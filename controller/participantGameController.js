...
const create = (req, res) => {
    const { id_lg, name_pg, number_pg, mail_pg, phone_pg, payment_pg } = req.body;
    const photo_pg = `${req.file.filename}`;

    if (!id_lg || !name_pg || !number_pg || !phone_pg) {
    ...
    const sql = "INSERT INTO participant_game (id_lg, name_pg, number_pg, photo_pg, mail_pg, phone_pg, payment_pg, dateInscription_pg) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    const values = [id_lg, name_pg, number_pg, photo_pg, mail_pg, phone_pg, payment_pg, dateInscription_pg];
...
const photo = [upload.single('img'), create]
