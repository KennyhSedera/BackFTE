...
exports.create = (req, res) => {
    const sql = "INSERT INTO participant_hackaton (id_Ph, nom_Ph, nombreRersonne_Ph, chefDeGroupe_Ph, tel_Ph, phone_Ph, payment_Ph) VALUES (?, ?, ?, ?, ?, ?, ?)";
    const values = req.body;
    db.query(sql, values, (err, result) => {
        if (err) return res.send(err);
        return res.send({ succee: 'Participant ajoutée avec succèe.' });
    })
}
...
function validateParticipantData(data) {
  ...
  exports.update = (req, res) => {
    const sql = "UPDATE participant_Hackaton SET `id_Ph`=?, `nom_Ph`=?, `nombrePersonne_Ph`=?, `chefDeGroupe_Ph`=?, `tel_Ph`=?, `phone_ Ph`=?, 'payment_Ph'= ? WHERE id_Ph=?";
    const id_Ph = req.params.id_Ph;
    const data = req.body;
